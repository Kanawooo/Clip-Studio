...
export interface PiVideoSession {
...
function assertRequiredToolsActive(session: AgentSession, required: readonly string[]): void {
  const active = new Set(session.getActiveToolNames());
  const missing = required.filter((name) => !active.has(name));
  if (missing.length > 0) throw new Error(`Pi 原生工具不可用：${missing.join(", ")}`);
}
