...
export function createApp(options: AppOptions): {
  ...
  const server = createServer((req, res) => {
    if (!setLocalRequestHeaders(req, res)) return;
  ...
  async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (outputContentMatch && (req.method === "GET" || req.method === "HEAD")) {
      handleStreamTaskOutput(
        taskManager,
        decodeURIComponent(outputContentMatch[1]),
        decodeURIComponent(outputContentMatch[2]),
        req,
        res,
      );
      return;
...
function health(projectRoot: string, startupId?: string, frontendBuildId?: string): Record<string, unknown> {
...
function setLocalRequestHeaders(req: IncomingMessage, res: ServerResponse): boolean {
