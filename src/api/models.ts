...
import type { ModelProtocol } from "../tasks/types.js";
import { MODEL_THINKING_LEVELS, type ModelThinkingLevel } from "../tasks/types.js";
...
import { ModelCapabilityError, ModelCapabilityRegistry } from "./model-capabilities.js";
import type { LocalStateStore } from "./local-state.js";
import type { ModelCatalogItem } from "../pi/model.js";

export function handleListModels(res: ServerResponse, models: readonly ModelCatalogItem[]): void {
...
export async function handleSetThinkingCapability(
  req: IncomingMessage,
  res: ServerResponse,
  registry: ModelCapabilityRegistry,
  localState: LocalStateStore,
  ...
  try {
    ...
    apiKey = config.apiKey;
    if (!Array.isArray(raw.levels) || raw.levels.some((level) => !MODEL_THINKING_LEVELS.includes(level as ModelThinkingLevel))) {
      throw new HttpError(400, "levels must contain only supported standard thinking levels");
    }
    const evidence = registry.configureThinking(
      raw.capabilityId,
      config,
      raw.levels as ModelThinkingLevel[],
    );
    await registry.persist();
...
function requiredDiscoveryString(value: unknown, name: string): string {
...
function discoveryProtocol(value: unknown): ModelProtocol {
