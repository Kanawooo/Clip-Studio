...
function initializeEnvironment(): void {
  ...
  if (major < 22 || (major === 22 && minor < 19)) {
    console.error(
      `[clip-studio] Node.js >= 22.19.0 is required (found ${process.version}). Exiting.`,
    );
    process.exit(1);
  }
  ...
  if (process.env.PI_VIDEO_RUNTIME_VERIFIED !== "1") {
    ...
    if (missing.length > 0) {
      ...
      console.error(
        "[clip-studio] Install the missing tools and make sure they are on PATH before starting the backend.",
      );
      process.exit(1);
    }
  } else {
