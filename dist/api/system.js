...
export async function handleReveal(manager, req, res) {
    try {
        ...
        if (!task)
            throw new HttpError(404, `task not found: ${raw.taskId}`);
...
async function revealInExplorer(targetPath) {
    ...
    await new Promise((resolve, reject) => {
        ...
        child.once("close", (code) => {
            ...
            if (code === 0 && stdout.includes("REVEALED"))
                resolve();
            else
                reject(new Error(stderr.trim() || "未能将资源管理器显示到前方"));
        });
    });
}
export function buildRevealScript(targetPath, isFile) {
...
export async function handleDialogPick(req, res) {
    try {
        const body = await readJsonBody(req);
...
function runPowerShellPicker(req, command) {
    return new Promise((resolve, reject) => {
        const child = spawn("powershell.exe", pickerPowerShellArgs(command), { windowsHide: true, stdio: ["ignore", "pipe", "ignore"], env: pickerEnvironment() });
        let stdout = "";
        child.stdout.setEncoding("utf8");
        child.stdout.on("data", (chunk) => { stdout += chunk; });
        child.once("error", reject);
        child.once("close", (code) => {
            ...
            if (result.type === "picked")
                ...
                reject(new Error(`无法打开系统选择窗口：${result.message}`));
            else
                reject(new Error(code === 0 ? "系统选择窗口没有返回结果" : "系统选择窗口启动失败"));
        });
        req.once("aborted", () => child.kill());
    });
}
export function buildDirectoryPickerScript(title, initialPath) {
    return buildWindowsPickerScript("directory", title, "", initialPath);
}
export function buildFilePickerScript(title, filter, initialPath) {
    return buildWindowsPickerScript("file", title, filter, initialPath);
}
export function pickerPowerShellArgs(command) {
    return ["-NoLogo", "-NoProfile", "-STA", "-EncodedCommand", Buffer.from(command, "utf16le").toString("base64")];
}
export function parsePickerOutput(output) {
...
function buildWindowsPickerScript(type, title, filter, initialPath) {
    return `
$ErrorActionPreference = 'Stop'
...
export function resolvePickerInitialPath(type, value) {
    if (typeof value !== "string" || !value.trim() || value.includes("\0") || value.length > 32_767)
        return undefined;
    try {
...
function powerShellUtf8Value(value) {
...
function compactPickerError(value) {
